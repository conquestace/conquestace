...
| `LLMControls.astro`                   | Sends prompt + instructions to the backend:<br>• Optional **extra instructions** textarea.<br>• Toggle whether instructions go at the **start** or **end** of the system prompt.<br>• **Edit system prompts** using presets (`danbooru`, `natural-language`, custom).<br>• POSTs everything to `/.netlify/functions/generatePrompt` using your `LLMConfig` settings.
