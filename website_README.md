...
| `LLMControls.astro`                   | Sends prompts to the backend:<br>• Toggle whether instructions go at the **start** or **end** of the system prompt.<br>• Uses the selected system‑prompt preset (`danbooru`, `natural-language`, custom).<br>• POSTs everything to `/.netlify/functions/generatePrompt` using your `LLMConfig` settings.
